...
public class Lecture {
	...
	//2. returnsAList(5) -> [1, 2, 3, 4, 5] **.add() function
	...
	//3. returnThirdName(["Frodo","Sam","Merry","Pippin"]) -> "Merry" **.get() function
	...
	//   onlyOneAnswer(["no"]) -> true **.size() function
	public boolean onlyOneAnswer(List<String> answers) {
		return answers.size() == 1;
	}

	//6. removeLastNumber([1.5, 2.2, 0.9]) -> [1.5, 2.2] **.remove()
	...
	//   hasACow(["turkey", "rooster"]) -> false **.contains() function
	public boolean hasACow(List<String> farmAnimals) {
		return farmAnimals.contains("cow");
	}

	//8. yourPlaceInLine(["customer","customer","you","customer"]) -> 3 **.indexOf() function
	...
	public String[] trimArray(String[] input) {
		//List<String> fixedSizedCopy = Arrays.asList(input);
